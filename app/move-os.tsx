...
import { MAX_PINNED, currentFocus, planContext, stageOf, triggerLabel } from "./domain/plans";
...
export function MoveOS(){
  ...
  const [selectedPlan,setSelectedPlan]=useState("secure-home");
  ...
  return <div className="app-shell">
    <header className="site-header">
      ...
      <div className="header-tools">
        ...
        <div className="utility-menu"><button aria-label="Open utilities">•••</button><div>{[...data.plans].sort((a,b)=>a.order-b.order).map(plan=><button key={plan.id} onClick={()=>{setSelectedPlan(plan.id);openPage("Plan")}}>{plan.title}</button>)}<button onClick={()=>openPage("FullPlan")}>View full plan</button><button onClick={()=>openPage("CashFlow")}>Cash Flow</button><button onClick={()=>openPage("Apartments")}>Apartment Search</button><button onClick={()=>openPage("References")}>References</button><button onClick={()=>openPage("Settings")}>Settings</button></div></div>
        <button className="quick-add" onClick={()=>setCaptureOpen(true)}>＋ Capture</button>
      </div>
    </header>
    <main>
      {page==="Home"&&<HomeDashboard data={data} update={setData} editItem={setEditing} openPlan={id=>{setSelectedPlan(id);openPage("Plan")}} openPreMove={()=>openPage("Pre-Move")} openCashFlow={()=>openPage("CashFlow")} openApartments={()=>openPage("Apartments")} openReferences={()=>openPage("References")}/>}
      {page==="Pre-Move"&&<PreMovePage data={data} update={setData} edit={setEditing} add={()=>setAdding(newItem("Task","Pre-Move"))} go={openPage}/>}
      {page==="Post-Move"&&<PhasePage phase={page} data={data} update={setData} edit={setEditing} add={(type,area)=>setAdding({...newItem(type,page),...(area?{workArea:area}:{})})} go={openPage}/>}
      ...
      {page==="Plan"&&<PlanDetail key={selectedPlan} planId={selectedPlan} data={data} update={setData} edit={setEditing} addTask={partial=>setAdding({...newItem("Task","Pre-Move"),...partial})} back={()=>openPage("Home")} openCashFlow={()=>openPage("CashFlow")} openApartments={()=>openPage("Apartments")}/>}
    ...
    {searchOpen&&<Search data={data} close={()=>setSearchOpen(false)} edit={item=>{setEditing(item);setSearchOpen(false)}} go={next=>{openPage(next);setSearchOpen(false)}} openPlan={id=>{setSelectedPlan(id);openPage("Plan");setSearchOpen(false)}}/>}
    {(editing||adding)&&<ItemEditor item={editing||adding!} all={data.items} data={data} onClose={()=>{setEditing(null);setAdding(null)}} onSave={saved=>{updateItem(saved);setEditing(null);setAdding(null)}} onDelete={editing?()=>{deleteItem(editing.id);setEditing(null)}:undefined} onOpen={setEditing}/>}
...
const stageCopy:Record<ActionStage,string>={Now:"Worth doing currently.",Next:"Likely to follow soon.",Triggered:"Waiting for something to happen first.",Later:"Captured, intentionally not current."};

function PreMovePage({data,update,edit,add,go}:{data:MoveData;update:(data:MoveData)=>void;edit:(item:MoveItem)=>void;add:()=>void;go:(page:Page)=>void}){
  const tasks=data.items.filter(item=>item.phase==="Pre-Move"&&isAction(item));
  const focusIds=new Set(currentFocus(data,5).map(item=>item.id));
  const open=tasks.filter(item=>!isDone(item)&&!focusIds.has(item.id));
  const ordered=(list:MoveItem[])=>[...list].sort((a,b)=>taskScore(b,data.items)-taskScore(a,data.items));
  const stage=(value:ActionStage)=>ordered(open.filter(item=>stageOf(item,data.items)===value));
  const triggered=stage("Triggered");
  const groups=new Map<string,MoveItem[]>();
  triggered.forEach(item=>{const label=triggerLabel(item,data.items,data.planRequirements);groups.set(label,[...(groups.get(label)||[]),item])});
  const done=tasks.filter(isDone).sort((a,b)=>(b.completedAt||b.updatedAt).localeCompare(a.completedAt||a.updatedAt));
  const line=(item:MoveItem)=><TaskLine key={item.id} item={item} data={data} update={update} edit={edit}/>;
  return <div className="page pre-move-page">
    <header className="simple-phase-head"><p className="kicker">PRE-MOVE</p><h1>Everything that has to happen before leaving, in the order it matters.</h1></header>
    <CurrentFocus data={data} update={update} edit={edit} openPreMove={()=>go("FullPlan")} linkLabel="Full plan →"/>
    {(["Now","Next"] as ActionStage[]).map(value=>{const list=stage(value);return list.length?<section key={value} className="stage-block"><header><h2>{value}</h2><p>{stageCopy[value]}</p></header><div className="focus-list">{list.map(line)}</div></section>:null})}
    {groups.size>0&&<section className="stage-block"><header><h2>Triggered / waiting</h2><p>{stageCopy.Triggered}</p></header>{[...groups.entries()].map(([trigger,list])=><div key={trigger} className="trigger-group"><h3>{trigger}</h3><div className="focus-list">{list.map(line)}</div></div>)}</section>}
    {stage("Later").length>0&&<details className="stage-block collapsible"><summary><h2>Later</h2><span>{stage("Later").length} items</span></summary><div className="focus-list">{stage("Later").map(line)}</div></details>}
    {done.length>0&&<details className="stage-block collapsible"><summary><h2>Done</h2><span>{done.length} items</span></summary><div className="focus-list">{done.map(line)}</div></details>}
    <button className="simple-add" onClick={add}>＋ Add a task</button>
  </div>;
}
...
function FlatWorkItem({item,all,data,update,edit}:{item:MoveItem;all:MoveItem[];data:MoveData;update:(data:MoveData)=>void;edit:(item:MoveItem)=>void}){
  ...
  return <TaskRow item={item} all={all} check={toggle} edit={()=>edit(item)} context={planContext(item,data)} compact/>;
}
...
function TaskRow({item,all,check,edit,compact=false,context}:{item:MoveItem;all:MoveItem[];check:()=>void;edit:()=>void;compact?:boolean;context?:string}){
  const waiting=blocker(item,all);
  const meta=[context||surfaceArea(item),item.dueDate&&formatDate(item.dueDate),waiting||item.blocker,item.status==="In Progress"?"In progress":null,item.optional?"Optional":null].filter(Boolean).join(" · ");
...
function Search({data,close,edit,go,openPlan}:{data:MoveData;close:()=>void;edit:(item:MoveItem)=>void;go:(page:Page)=>void;openPlan:(id:string)=>void}){
  ...
  const planHits=normalized?[...data.plans.map(plan=>({id:plan.id,planId:plan.id,title:plan.title,copy:plan.question,resultType:"Plan"})),...data.planQuestions.map(question=>({id:question.id,planId:question.planId,title:question.question,copy:question.answer||"Open question",resultType:"Question"})),...data.planRoutes.map(route=>({id:route.id,planId:route.planId,title:route.title,copy:route.summary||route.status,resultType:"Route"})),...data.planRequirements.map(requirement=>({id:requirement.id,planId:requirement.planId,title:requirement.title,copy:requirement.status,resultType:"Requirement"}))].filter(hit=>hit.planId&&`${hit.title} ${hit.copy}`.toLowerCase().includes(normalized)).slice(0,6):[];
...
function ItemEditor({item,all,data,onClose,onSave,onDelete,onOpen}:{item:MoveItem;all:MoveItem[];data:MoveData;onClose:()=>void;onSave:(item:MoveItem)=>void;onDelete?:()=>void;onOpen:(item:MoveItem)=>void}){
  const [draft,setDraft]=useState(item);
  const planSections=data.planSections.filter(section=>section.planId===draft.planId).sort((a,b)=>a.order-b.order);
  const planRoutes=data.planRoutes.filter(route=>route.planId===draft.planId);
  const planRequirements=data.planRequirements.filter(requirement=>requirement.planId===draft.planId);
  const pinnedElsewhere=data.items.filter(candidate=>candidate.pinnedToFocus&&candidate.id!==draft.id&&!isDone(candidate)).length;
  ...
  return <div className="overlay" onMouseDown={event=>event.currentTarget===event.target&&onClose()}><form className={`modal item-editor simple-editor type-${draft.type.toLowerCase()}`} onSubmit={save}>
    ...
    <section className="editor-quick"><Field label={draft.type==="Task"?"What needs doing?":draft.type==="Project"?"Project name":"Goal name"}><input autoFocus required value={draft.title} onChange={event=>setDraft({...draft,title:event.target.value})}/></Field><div className="form-grid three"><Field label="Area"><select value={draft.workArea} onChange={event=>setDraft({...draft,workArea:event.target.value as WorkArea})}>{areas.map(area=><option key={area}>{area}</option>)}</select></Field>{draft.type==="Task"&&<Field label="Due date"><input type="date" value={draft.dueDate||""} onChange={event=>setDraft({...draft,dueDate:event.target.value||undefined})}/></Field>}<Field label="Status"><select value={draft.status} onChange={event=>setDraft({...draft,status:event.target.value as Status})}>{statuses.map(status=><option key={status}>{status}</option>)}</select></Field></div>
      <div className="form-grid"><Field label="Plan"><select value={draft.planId||""} onChange={event=>setDraft({...draft,planId:event.target.value||undefined,planSectionId:undefined,routeId:undefined,requirementId:undefined})}><option value="">Not part of a plan</option>{[...data.plans].sort((a,b)=>a.order-b.order).map(plan=><option key={plan.id} value={plan.id}>{plan.title}</option>)}</select></Field>{planSections.length>0&&<Field label="Part of the plan"><select value={draft.planSectionId||""} onChange={event=>setDraft({...draft,planSectionId:event.target.value||undefined})}><option value="">Whole plan</option>{planSections.map(section=><option key={section.id} value={section.id}>{section.title}</option>)}</select></Field>}</div>
      {draft.type==="Task"&&<div className="form-grid"><Field label="Action timing"><select value={draft.actionStage||stageOf(draft,all)} onChange={event=>setDraft({...draft,actionStage:event.target.value as ActionStage})}><option value="Now">Now</option><option value="Next">Next</option><option value="Triggered">When something happens</option><option value="Later">Later</option></select></Field>{(draft.actionStage||stageOf(draft,all))==="Triggered"?<Field label="When…"><input value={draft.triggerText||""} placeholder="When the lease is signed" onChange={event=>setDraft({...draft,triggerText:event.target.value||undefined})}/></Field>:<label className="switch compact"><div><b>Pin to Current Focus</b><small>{!draft.pinnedToFocus&&pinnedElsewhere>=MAX_PINNED?`Already ${MAX_PINNED} pinned—unpin one first.`:"Pinned tasks outrank suggestions."}</small></div><input type="checkbox" disabled={!draft.pinnedToFocus&&pinnedElsewhere>=MAX_PINNED} checked={!!draft.pinnedToFocus} onChange={event=>setDraft({...draft,pinnedToFocus:event.target.checked})}/></label>}</div>}
      {draft.type==="Task"&&(planRoutes.length>0||planRequirements.length>0)&&<div className="form-grid">{planRoutes.length>0&&<Field label="Route"><select value={draft.routeId||""} onChange={event=>setDraft({...draft,routeId:event.target.value||undefined})}><option value="">No specific route</option>{planRoutes.map(route=><option key={route.id} value={route.id}>{route.title}</option>)}</select></Field>}{planRequirements.length>0&&<Field label="Advances requirement"><select value={draft.requirementId||""} onChange={event=>setDraft({...draft,requirementId:event.target.value||undefined})}><option value="">None in particular</option>{planRequirements.map(requirement=><option key={requirement.id} value={requirement.id}>{requirement.title}</option>)}</select></Field>}</div>}
    </section>
