...
function PlanSearch({data,onClose,onOpen,goTo}:{data:MoveData;onClose:()=>void;onOpen:(item:MoveItem)=>void;goTo:(page:Page)=>void}){
  ...
  return <div className="search-backdrop" onMouseDown={event=>event.currentTarget===event.target&&onClose()}><section className="plan-search" role="dialog" aria-modal="true" aria-label="Find anything in Randy's move plan"><header><span>⌕</span><input autoFocus value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search tasks, sections, notes, or statuses…"/><button onClick={onClose} aria-label="Close search">×</button></header><div className="search-area-shortcuts">{(["Clear","Build","Become"] as const).map((area,index)=><button key={area} onClick={()=>goTo(area)}><span>0{index+1}</span>{areaLabels[area]}</button>)}</div><div className="search-results"><small>{normalized?`${results.length} MATCH${results.length===1?"":"ES"}`:"OPEN REQUIRED ACTIONS"}</small>{results.length?results.map(item=><button key={item.id} onClick={()=>onOpen(item)}><span className={`search-area search-${item.area.toLowerCase()}`}>{item.kind==="Reference"?"FYI":item.kind==="Reflection"?"NOTE":item.area.slice(0,1)}</span><div><b>{item.title}</b><small>{item.kind==="Reference"?"Decision guidance · not scored":item.kind==="Reflection"?"Discovery / inner work · not scored":`${sectionLabels[item.section]||item.section} · ${item.optional?"Optional":item.status}`}</small></div><i>→</i></button>):<p>No pieces match that search.</p>}</div><footer><span><kbd>ESC</kbd> close</span><p>One place to remember everything.</p></footer></section></div>
}
...
function AreaDashboard({area,data,update,goTo}:{area:Exclude<Area,"Vault">;data:MoveData;update:(d:MoveData)=>void;goTo:(page:Page)=>void}) {
  ...
  return <section className={`area-dashboard dashboard-${area.toLowerCase()}`}><div className="area-dashboard-summary"><div className="area-definition"><small>{meanings.label}</small><p>{meanings.definition}</p></div><div className={`area-completion progress-${percent<35?"low":percent<75?"medium":"high"}`}><strong>{percent}%</strong><span>{complete} of {items.length} required actions complete</span></div><div className="area-reflection-count"><strong>{reflectionDone}/{reflectionTotal}</strong><span>reflections answered</span></div></div><nav className="area-toc grouped-toc" aria-label={`${areaLabels[area]} section overview`}><small>JUMP TO THE KIND OF WORK YOU NEED</small><div className="toc-groups">{areaGroups[area].map((group,groupIndex)=><section key={group.title} className="toc-group"><header><span>0{groupIndex+1}</span><div><b>{group.title}</b><small>{group.copy}</small></div></header><div>{group.sections.map(section=>{const sectionItems=data.items.filter(i=>i.area===area&&i.section===section&&!i.optional&&isAction(i));const optional=data.items.filter(i=>i.area===area&&i.section===section&&i.optional&&isAction(i)).length;const sectionReflections=data.items.filter(i=>i.area===area&&i.section===section&&i.kind==="Reflection").length;const done=sectionItems.filter(isDone).length;return <article className="toc-row" key={section}><button className="toc-jump" onClick={()=>document.getElementById(sectionId(section))?.scrollIntoView({behavior:"smooth",block:"start"})}><div><b>{sectionLabels[section]||section}</b><small>{sectionItems.length?`${done}/${sectionItems.length} actions`:"Information and reflection"}{optional?` · ${optional} optional`:""}{sectionReflections?` · ${sectionReflections} reflection${sectionReflections===1?"":"s"}`:""}</small></div><i>↓</i></button><label className="section-target" title="Working target date"><input aria-label={`${sectionLabels[section]||section} target date`} type="date" value={data.sectionTargets[section]||""} onChange={e=>update({...data,sectionTargets:{...data.sectionTargets,[section]:e.target.value}})}/></label></article>})}{group.apartmentTool&&<button className="toc-tool-link" onClick={()=>goTo("Apartments")}><span>TOOL</span><div><b>Apartment Matrix</b><small>Compare price, fit, qualification rules, sublets, and specials.</small></div><i>→</i></button>}</div></section>)}</div></nav></section>;
}
...
function AreaPage({area,data,update,edit,goTo}:{area:Area;data:MoveData;update:(d:MoveData)=>void;edit:(i:MoveItem)=>void;goTo:(p:Page)=>void}) {
  ...
  const renderSection=(section:string)=>{
    ...
    return <details id={sectionId(section)} className="section-card section-anchor" key={section} open={sectionIndex===1}>
      <summary><div className="section-number"><span>0{sectionIndex}</span></div><div className="section-summary"><h2>{sectionLabels[section]||section}</h2><p>{sectionCopy[section] || reflectiveCopy(section)}</p></div><div className="section-count"><b>{requiredItems.filter(isDone).length}/{requiredItems.length}</b><small>required{reflections.length?` · ${reflections.filter(isDone).length}/${reflections.length} reflections`:""}</small></div><i>⌄</i></summary>
      <div className="section-body">{references.length>0&&<aside className="section-guidance section-guidance-top"><small>FYI · READ BEFORE CHOOSING OR ACTING</small>{references.map(reference=><button key={reference.id} onClick={()=>edit(reference)}><span>FYI</span><div><b>{reference.title}</b><small>{reference.description}</small></div><i>→</i></button>)}</aside>}{reflections.length>0&&<aside className="section-reflections section-reflections-top"><small>REFLECTIONS · TRACKED SEPARATELY FROM TASKS</small>{reflections.map(reflection=><button key={reflection.id} onClick={()=>edit(reflection)}><span>{isDone(reflection)?"✓":"NOTE"}</span><div><b>{reflection.title}</b><small>{reflection.description}</small></div><em>{reflection.status}</em><i>→</i></button>)}</aside>}<div className="section-action-heading"><div><small>REQUIRED ACTIONS</small><b>{reason?.why||"Work that creates a clear, useful result."}</b></div>{reason&&<span>Unlocks: {reason.unlocks}</span>}</div>{requiredItems.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}{optionalItems.length>0&&<details className="optional-group"><summary><span>OPTIONAL</span><div><b>{optionalItems.length} linked action{optionalItems.length===1?"":"s"}</b><small>Useful if circumstances call for them; hidden from required progress.</small></div><i>⌄</i></summary><div>{optionalItems.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}</div></details>}<button className="add-row" onClick={()=>add(section)}>＋ Add an action</button></div>
    </details>;
  };
...
function IncomeFoundation({data,update,edit}:{data:MoveData;update:(d:MoveData)=>void;edit:(item:MoveItem)=>void}){
  ...
  return <section className="income-foundation section-anchor">
    ...
    {references.length>0&&<aside className="section-guidance section-guidance-top"><small>FYI · USE WHILE CHOOSING A ROUTE</small>{references.map(reference=><button key={reference.id} onClick={()=>edit(reference)}><span>FYI</span><div><b>{reference.title}</b><small>{reference.description}</small></div><i>→</i></button>)}</aside>}
    <div className="income-actions"><small>THE DELIVERABLES</small>{required.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}</div>
    {optional.length>0&&<details className="optional-group"><summary><span>OPTIONAL</span><div><b>{optional.length} linked action{optional.length===1?"":"s"}</b><small>Available if the route calls for them; excluded from required progress.</small></div><i>⌄</i></summary><div>{optional.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}</div></details>}
