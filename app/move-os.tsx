...
function IncomeFoundation({data,update,edit}:{data:MoveData;update:(d:MoveData)=>void;edit:(item:MoveItem)=>void}){
  ...
  return <section className="income-foundation section-anchor">
    ...
    {references.length>0&&<aside className="section-guidance section-guidance-top"><header><span aria-hidden="true">i</span><div><b>Helpful information</b><small>Use this context while choosing an income route. It is not another deliverable.</small></div></header>{references.map(reference=><button key={reference.id} onClick={()=>edit(reference)}><div><b>{reference.title}</b><small>{reference.description}</small></div><span className="guidance-edit">Edit note</span></button>)}</aside>}
    <section className="action-mode income-actions"><header><span aria-hidden="true">✓</span><div><small>ACTIONS</small><b>Income deliverables</b><p>Each row should leave you with evidence, a decision, or a completed application step.</p></div><em>{required.filter(isDone).length} of {required.length} complete</em></header><div className="action-list">{required.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}</div>{optional.length>0&&<details className="optional-group"><summary><span>OPTIONAL</span><div><b>{optional.length} linked action{optional.length===1?"":"s"}</b><small>Available if the route calls for them; excluded from required progress.</small></div><i>⌄</i></summary><div>{optional.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}</div></details>}</section>
