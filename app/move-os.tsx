...
function CalendarPage({data,edit}:{data:MoveData;edit:(i:MoveItem)=>void}) {
  ...
  const groups=[
    {month:"AUG",name:"Ground & gather",symbol:"☿",anchor:"2026-08-03",copy:"Turn fears into facts. Keep the options moving.",items:data.items.filter(i=>i.timing==="Now"&&!isDone(i)&&isAction(i)).slice(0,7)},
...
function ConnectionsPage({data,edit,goTo}:{data:MoveData;edit:(i:MoveItem)=>void;goTo:(p:Page)=>void}) {
  ...
  return <div className="page connections-page">
    ...
    </section>:<section className={`constellation-zoom constellation-${selectedNode.area.toLowerCase()}`}><header><button onClick={()=>setSelectedStar(null)}>← All five stars</button><div><small>{selectedNode.orbit} · FOCUSED CONSTELLATION</small><h2>{selectedNode.title}</h2><p>{selectedNode.copy}</p></div></header><div className="zoom-sky"><div className="zoom-center"><button className="zoom-main" onClick={()=>selectedMain&&edit(selectedMain)}><span>{selectedNode.symbol}</span><small>MAIN PIECE</small><b>{selectedMain?.title||selectedNode.title}</b><em>{selectedMain?.status}</em></button></div>{directChildren.length>0&&<div className="zoom-branch direct"><small>SMALLER PIECES INSIDE THIS STAR</small>{directChildren.map(task=><button className={isDone(task)?"settled":""} key={task.id} onClick={()=>edit(task)}><span>✦</span><div><b>{task.title}</b><em>{task.status}</em></div></button>)}</div>}<div className="zoom-branch related"><small>RELATED ACTIONS IN THIS ORBIT</small>{relatedTasks.slice(0,14).map(task=><button className={isDone(task)?"settled":""} key={task.id} onClick={()=>edit(task)}><span>·</span><div><b>{task.title}</b><em>{task.parentId?`Subtask of ${data.items.find(i=>i.id===task.parentId)?.title||"another piece"}`:task.section}</em></div></button>)}</div></div><p className="constellation-help">The center is the main piece. Its smaller pieces branch directly from it; related actions remain nearby. Select any star to edit it.</p></section>}
