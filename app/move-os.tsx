...
function AreaPage({area,data,update,edit,goTo}:{area:Area;data:MoveData;update:(d:MoveData)=>void;edit:(i:MoveItem)=>void;goTo:(p:Page)=>void}) {
  ...
  const renderSection=(section:string)=>{
    ...
    return <details id={sectionId(section)} className="section-card section-anchor" key={section} open={sectionIndex===1}>
      ...
      <div className="section-body">{references.length>0&&<aside className="section-guidance section-guidance-top"><header><span aria-hidden="true">i</span><div><b>Helpful information</b><small>Context to keep in mind—not another task to complete.</small></div></header>{references.map(reference=><button key={reference.id} onClick={()=>edit(reference)}><div><b>{reference.title}</b><small>{reference.description}</small></div><span className="guidance-edit">Edit note</span></button>)}</aside>}{reflections.length>0&&<aside className="section-reflections section-reflections-top"><small>REFLECTIONS · TRACKED SEPARATELY FROM TASKS</small>{reflections.map(reflection=><button key={reflection.id} onClick={()=>edit(reflection)}><span aria-hidden="true">{isDone(reflection)?"✓":"✦"}</span><div><b>{reflection.title}</b><small>{reflection.description}</small></div><em>{reflection.status}</em><i>→</i></button>)}</aside>}<div className="section-action-heading"><div><small>REQUIRED ACTIONS</small><b>{reason?.why||"Work that creates a clear, useful result."}</b></div></div>{requiredItems.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}{optionalItems.length>0&&<details className="optional-group"><summary><span>OPTIONAL</span><div><b>{optionalItems.length} linked action{optionalItems.length===1?"":"s"}</b><small>Useful only if circumstances call for it. It does not count toward required progress.</small></div><i>⌄</i></summary><div>{optionalItems.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}</div></details>}<button className="add-row" onClick={()=>add(section)}>＋ Add an action</button></div>
    </details>;
  };
