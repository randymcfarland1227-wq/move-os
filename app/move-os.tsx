...
function PlanSearch({data,onClose,onOpen,goTo}:{data:MoveData;onClose:()=>void;onOpen:(item:MoveItem)=>void;goTo:(page:Page)=>void}){
  ...
  const results=normalized?data.items.filter(item=>[item.title,item.description,item.section,item.notes,item.status,item.knowledgeStatus].some(value=>value?.toLowerCase().includes(normalized))).slice(0,12):data.items.filter(item=>isAction(item)&&!isDone(item)&&item.timing!=="Allowed to wait").slice(0,8);
  return <div className="search-backdrop" onMouseDown={event=>event.currentTarget===event.target&&onClose()}><section className="plan-search" role="dialog" aria-modal="true" aria-label="Find anything in Move OS"><header><span>⌕</span><input autoFocus value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search tasks, sections, notes, or statuses…"/><button onClick={onClose} aria-label="Close search">×</button></header><div className="search-area-shortcuts">{(["Clear","Build","Become"] as const).map(area=><button key={area} onClick={()=>goTo(area)}><span>{area==="Clear"?"↗":area==="Build"?"◇":"✦"}</span>{area}</button>)}</div><div className="search-results"><small>{normalized?`${results.length} MATCH${results.length===1?"":"ES"}`:"OPEN ACTIONS"}</small>{results.length?results.map(item=><button key={item.id} onClick={()=>onOpen(item)}><span className={`search-area search-${item.area.toLowerCase()}`}>{item.kind==="Reference"?"PSA":item.area.slice(0,1)}</span><div><b>{item.title}</b><small>{item.kind==="Reference"?"Decision guidance · does not count as a task":`${item.section} · ${item.status}`}</small></div><i>→</i></button>):<p>No pieces match that search.</p>}</div><footer><span><kbd>ESC</kbd> close</span><p>One place to remember everything.</p></footer></section></div>
}
...
function Today({data,update,overwhelmed,setOverwhelmed,edit,goTo}:{data:MoveData;update:(d:MoveData)=>void;overwhelmed:boolean;setOverwhelmed:(v:boolean)=>void;edit:(i:MoveItem)=>void;goTo:(p:Page)=>void}) {
  ...
  const settled=data.items.filter(i=>isAction(i)&&isDone(i)).slice(-3).reverse();
  ...
  const active=data.items.filter(i=>i.timing!=="Allowed to wait"&&isAction(i));
  ...
  if(overwhelmed) {
    ...
    const ignore=data.items.find(i=>isAction(i)&&i.timing==="Allowed to wait");
...
function ProgressHub({data,edit,goTo}:{data:MoveData;edit:(i:MoveItem)=>void;goTo:(p:Page)=>void}) {
  ...
  const actionable=data.items.filter(i=>i.timing!=="Allowed to wait"&&isAction(i));
...
function CalendarPage({data,update,edit}:{data:MoveData;update:(d:MoveData)=>void;edit:(i:MoveItem)=>void}) {
  ...
  const groups=[
    {key:"ground",month:"AUG",name:"Ground & gather",symbol:"☿",copy:"Turn fears into facts. Keep the options moving.",items:data.items.filter(i=>i.timing==="Now"&&!isDone(i)&&isAction(i)).slice(0,7)},
    {key:"test",month:"SEP",name:"Test & decide",symbol:"◐",copy:"Income conversations, applications, property rules, and a real shortlist.",items:data.items.filter(i=>isAction(i)&&(i.dueDate?.startsWith("2026-09")||["property-rules","apartment-shortlist","rent-budget"].includes(i.id))).slice(0,7)},
    {key:"commit",month:"OCT",name:"Commit & cross",symbol:"○",copy:"The October 15–31 window. Sign only when enough is known.",items:data.items.filter(i=>isAction(i)&&(i.stage==="Commit"||["holiday-plan","goodbye-time","grandma-letter"].includes(i.id))).slice(0,7)},
...
function ConnectionsPage({data,edit,goTo}:{data:MoveData;edit:(i:MoveItem)=>void;goTo:(p:Page)=>void}) {
  ...
  const selectedTasks=(!selectedStar?[]:selectedStar==="credit-plan"?data.items.filter(i=>i.area==="Clear"):selectedStar==="rental-ready"?data.items.filter(i=>["Rental Readiness","Housing and Lease"].includes(i.section)):selectedStar==="income-options"?data.items.filter(i=>i.section==="Post-Move Income"):selectedStar==="lease"?data.items.filter(i=>["Housing and Lease","Physical Move","Land Softly"].includes(i.section)):data.items.filter(i=>i.area==="Become")).filter(isAction);
...
function AreaDashboard({area,data,update}:{area:Exclude<Area,"Vault">;data:MoveData;update:(d:MoveData)=>void}) {
  ...
  const items=data.items.filter(i=>i.area===area&&i.timing!=="Allowed to wait"&&isAction(i));
...
function AreaPage({area,data,update,edit}:{area:Area;data:MoveData;update:(d:MoveData)=>void;edit:(i:MoveItem)=>void}) {
  ...
  return <div className={`page area-page area-${area.toLowerCase()}`}>
    ...
    {areaSections[area].filter(section=>!(area==="Become"&&section==="My Fuel Source")).map((section,index)=>{
      ...
      const items=data.items.filter(i=>i.area===area&&i.section===section&&!i.parentId&&isAction(i));
      ...
      return <details id={sectionId(section)} className={`section-card section-anchor ${locked?"locked":""}`} key={section} open={index<2&&!locked}>
        ...
        <div className="section-body">{items.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}{references.length>0&&<aside className="section-guidance"><small>DECISION GUIDANCE · NOT ACTION ITEMS</small><p>Keep these in view when making the actions above. They do not affect completion.</p>{references.map(reference=><button key={reference.id} onClick={()=>edit(reference)}><span>PSA</span><div><b>{reference.title}</b><small>{reference.description}</small></div><i>→</i></button>)}</aside>}{!locked&&<button className="add-row" onClick={()=>add(section)}>＋ Add something gently</button>}{locked&&<p className="locked-copy">There’s nothing to do here yet. A confirmed address and Move Day will unlock these logistics.</p>}</div>
      </details>;
    })}
  </div>;
}
...
function ItemModal({item,all,onOpen,onClose,onSave,onDelete}:{item:MoveItem;all:MoveItem[];onOpen:(i:MoveItem)=>void;onClose:()=>void;onSave:(i:MoveItem)=>void;onDelete:()=>void}) {
  ...
  const references=all.filter(i=>i.kind==="Reference"&&i.referenceFor?.includes(draft.id));
  return <div className="modal-backdrop cosmic-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)onClose()}}>
    <form className={`modal cosmic-editor editor-${draft.area.toLowerCase()}`} onSubmit={e=>{e.preventDefault();onSave({...draft,updatedAt:new Date().toISOString().slice(0,10)})}}>
      ...
      <section className="editor-form">
        <div className="modal-head"><div><span>{draft.kind==="Reference"?"DECISION GUIDANCE · NOT AN ACTION":item.title?"EDITING A LIVING PIECE":"ADDING A NEW PIECE"}</span><h2>{item.title?draft.title||"Untitled piece":"Add something"}</h2></div><button type="button" onClick={onClose}>×</button></div>
