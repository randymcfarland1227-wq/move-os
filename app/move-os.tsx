...
function Today({data,update,overwhelmed,setOverwhelmed,edit,goTo}:{data:MoveData;update:(d:MoveData)=>void;overwhelmed:boolean;setOverwhelmed:(v:boolean)=>void;edit:(i:MoveItem)=>void;goTo:(p:Page)=>void}) {
  ...
  return <div className="page today">
    ...
    <section className="overview-dashboard"><div className={`overview-score progress-${completion<35?"low":completion<75?"medium":"high"}`}><span>{completion}%</span><div><small>ACTIVE PLAN SETTLED</small><b>{complete} of {active.length} pieces</b></div></div><div className="overview-progress-bars">{areaProgress.map(stat=><button key={stat.area} onClick={()=>goTo(stat.area)}><div><b>{stat.area}</b><span>{stat.done}/{stat.total}</span></div><i><em style={{width:`${stat.percent}%`}}/></button>)}</div><blockquote><small>WHY THIS MATTERS</small><p>{data.profile.reason}</p></blockquote></section>
