...
export function MoveOS() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <label className="hide-completed-toggle"><input type="checkbox" checked={data.hideCompleted} onChange={event=>update({...data,hideCompleted:event.target.checked})}/><span><b>Hide completed</b><small>Finished work stays saved</small></span></label>
...
function PlanSearch({data,onClose,onOpen,goTo}:{data:MoveData;onClose:()=>void;onOpen:(item:MoveItem)=>void;goTo:(page:Page)=>void}){
  ...
  return <div className="search-backdrop" onMouseDown={event=>event.currentTarget===event.target&&onClose()}><section className="plan-search" role="dialog" aria-modal="true" aria-label="Find anything in Move OS"><header><span>⌕</span><input autoFocus value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search tasks, sections, notes, or statuses…"/><button onClick={onClose} aria-label="Close search">×</button></header><div className="search-area-shortcuts">{(["Clear","Build","Become"] as const).map(area=><button key={area} onClick={()=>goTo(area)}><span>{area==="Clear"?"↗":area==="Build"?"◇":"✦"}</span>{area}</button>)}</div><div className="search-results"><small>{normalized?`${results.length} MATCH${results.length===1?"":"ES"}`:"OPEN REQUIRED ACTIONS"}</small>{results.length?results.map(item=><button key={item.id} onClick={()=>onOpen(item)}><span className={`search-area search-${item.area.toLowerCase()}`}>{item.kind==="Reference"?"PSA":item.kind==="Reflection"?"✦":item.area.slice(0,1)}</span><div><b>{item.title}</b><small>{item.kind==="Reference"?"Decision guidance · not scored":item.kind==="Reflection"?"Discovery / inner work · not scored":`${item.section} · ${item.optional?"Optional":item.status}`}</small></div><i>→</i></button>):<p>No pieces match that search.</p>}</div><footer><span><kbd>ESC</kbd> close</span><p>One place to remember everything.</p></footer></section></div>
}
...
function CalendarPage({data,update,edit}:{data:MoveData;update:(d:MoveData)=>void;edit:(i:MoveItem)=>void}) {
  ...
  const daysUntil=(date:string)=>Math.max(0,Math.ceil((new Date(date+"T12:00").getTime()-today.getTime())/86400000));
  const groups=[
    {key:"ground",month:"NOW",name:"Income & credit",symbol:"☿",copy:"Replace Element income and begin the 625+ credit plan in parallel.",items:data.items.filter(i=>isAction(i)&&["income-evidence","remote-search","chicago-hybrid","credit-plan"].includes(i.id)).slice(0,7)},
    {key:"test",month:"NEXT",name:"Compare & verify",symbol:"◐",copy:"Build the rental packet, learn Chicago neighborhoods, and verify real property rules.",items:data.items.filter(i=>isAction(i)&&["application-packet","income-proof","lease"].includes(i.id)).slice(0,7)},
    {key:"commit",month:"OCT",name:"Commit & cross",symbol:"○",copy:"The October 15–31 window. Sign only when income, credit, and terms are credible.",items:data.items.filter(i=>isAction(i)&&(i.stage==="Commit"||["holiday-plan","lease"].includes(i.id))).slice(0,7)},
    {key:"land",month:"LAND",name:"Arrive softly",symbol:"✦",copy:"Address-dependent setup, a functional home, routines, and belonging.",items:data.items.filter(i=>isAction(i)&&(i.timing==="After the lease"||i.timing==="After arrival"||i.stage==="Land")).slice(0,7)},
  ];
  return <div className="page calendar-page"><div className="tool-hero"><div><p className="eyebrow">TIME AS A SUPPORT, NOT A THREAT</p><h1>Your move calendar.</h1><p className="lede">Each chapter has its own editable destination date. Change one without changing the others.</p></div><div className="moon-clock"><i/><span>OCT<br/><b>24</b></span></div></div><section className="calendar-orbit">{groups.map((group,index)=>{const target=data.calendarTargets[group.key]||data.profile.targetMoveDate;return <article key={group.month}><header><span>{group.symbol}</span><div><small>{group.month} · 0{index+1}</small><h2>{group.name}</h2></div></header><label className="calendar-target"><span>COUNTING DOWN TO</span><input type="date" value={target} onChange={e=>update({...data,calendarTargets:{...data.calendarTargets,[group.key]:e.target.value}})}/></label><div className="month-countdown"><strong>{daysUntil(target)}</strong><span>days from today<br/>to this chapter’s date</span></div><p>{group.copy}</p><div>{group.items.map(item=><button key={item.id} onClick={()=>edit(item)}><span className={isDone(item)?"done":""}>{isDone(item)?"✓":"○"}</span><div><b>{item.title}</b><small>{item.dueDate?new Date(item.dueDate+"T12:00").toLocaleDateString("en-US",{month:"short",day:"numeric"}):item.knowledgeStatus}</small></div></button>)}</div></article>})}</section><div className="calendar-note"><span>☼</span><p><b>The plan can breathe.</b> These four dates are independent working anchors. Changing one does not change your overall planning date or permission to move.</p></div></div>;
}
...
function ConnectionsPage({data,edit,goTo}:{data:MoveData;edit:(i:MoveItem)=>void;goTo:(p:Page)=>void}) {
  ...
  const nodes=[
    ...
    {area:"Build" as Page,symbol:"◇",title:"Build the rental packet",copy:"Documents, property rules, and a standard-lease versus sublet choice create a usable application path.",item:"application-packet",orbit:"Foundation orbit"},
    {area:"Build" as Page,symbol:"☿",title:"Replace the income",copy:"Remote and Chicago-hybrid searches move together; Denver stays a lighter backup.",item:"income-evidence",orbit:"Decision orbit"},
  ...
  const selectedTasks=(!selectedStar?[]:selectedStar==="credit-plan"?data.items.filter(i=>i.area==="Clear"):selectedStar==="application-packet"?data.items.filter(i=>["Rental Readiness","Preferences in Search","Getting a Place"].includes(i.section)):selectedStar==="income-evidence"?data.items.filter(i=>i.section==="Post-Move Income"||i.id==="income-proof"):selectedStar==="lease"?data.items.filter(i=>["Getting a Place","Physical Move — Pre","Physical Move","Post Arrival"].includes(i.section)):data.items.filter(i=>i.area==="Become")).filter(isAction);
  const selectedMain=data.items.find(i=>i.id===selectedStar);
  const directChildren=selectedTasks.filter(i=>i.parentId===selectedStar);
  const relatedTasks=selectedTasks.filter(i=>i.id!==selectedStar&&!directChildren.some(child=>child.id===i.id));
  const connectionLabel=(task:MoveItem)=>{const dependency=data.items.find(i=>i.id===task.dependency);const parent=data.items.find(i=>i.id===task.parentId);return dependency?{kind:"WAITING FOR",item:dependency}:parent?{kind:"PART OF",item:parent}:undefined};
  return <div className="page connections-page">
    ...
    {!selectedNode?<section className="constellation-map saturn-map">
      <div className="constellation-center saturn-center"><small>THE LIFE AT THE CENTER</small><strong>Safe. Alive.<br/>Creative. Mine.</strong><span>Chicago first · October 2026</span><i className="saturn-band band-a"/><i className="saturn-band band-b"/></div>
    ...
    </section>:<section className={`constellation-zoom constellation-${selectedNode.area.toLowerCase()}`}><header><button onClick={()=>setSelectedStar(null)}>← All five stars</button><div><small>{selectedNode.orbit} · FOCUSED CONSTELLATION</small><h2>{selectedNode.title}</h2><p>{selectedNode.copy}</p></div></header><div className="zoom-sky"><div className="zoom-center"><button className="zoom-main" onClick={()=>selectedMain&&edit(selectedMain)}><span>{selectedNode.symbol}</span><small>{selectedMain?.kind==="Reference"?"GUIDING IDEA":"MAIN TASK"}</small><b>{selectedMain?.title||selectedNode.title}</b><em>{selectedMain?.kind==="Reference"?"A compass for the actions around it":selectedMain?.status}</em></button></div>{directChildren.length>0&&<div className="zoom-branch direct"><small>SMALLER PIECES INSIDE THIS STAR</small>{directChildren.map(task=>{const link=connectionLabel(task);return <div className="connected-star" key={task.id}>{link&&<button className="dependency-chip" onClick={()=>edit(link.item)}><small>{link.kind}</small><b>{link.item.title}</b></button>}<button className={isDone(task)?"settled":""} onClick={()=>edit(task)}><span>✦</span><div><b>{task.title}</b><em>{task.status}</em></div></button></div>})}</div>}<div className="zoom-branch related"><small>RELATED ACTIONS IN THIS ORBIT</small>{relatedTasks.slice(0,14).map(task=>{const link=connectionLabel(task);return <div className="connected-star" key={task.id}>{link&&<button className="dependency-chip" onClick={()=>edit(link.item)}><small>{link.kind}</small><b>{link.item.title}</b></button>}<button className={isDone(task)?"settled":""} onClick={()=>edit(task)}><span>·</span><div><b>{task.title}</b><em>{task.section}</em></div></button></div>})}</div></div><div className="connection-key"><span><i>✦</i> Action you can complete</span><span><i>↳</i> Line to its parent or dependency</span><span><i>◌</i> Guiding ideas do not count as tasks</span></div><p className="constellation-help">Every visible connection is clickable. Select the attached label to open the prerequisite or parent; select the star to edit the action itself.</p></section>}
...
function AreaDashboard({area,data,update}:{area:Exclude<Area,"Vault">;data:MoveData;update:(d:MoveData)=>void}) {
  ...
  return <section className={`area-dashboard dashboard-${area.toLowerCase()}`}><div className="area-definition"><small>{meanings.label}</small><p>{meanings.definition}</p><div>{meanings.labels.map(label=><span key={label}>{label}</span>)}</div></div><div className="area-completion"><div className={`mini-progress progress-${percent<35?"low":percent<75?"medium":"high"}`}><strong>{percent}%</strong><small>{items.length?"complete":"reflective"}</small></div><p>{items.length?<><b>{complete} of {items.length}</b> required actions complete</>:<><b>No scored tasks.</b> This area holds reflection and guidance.</>}</p></div><nav className="area-toc" aria-label={`${area} table of contents`}><small>SECTION OVERVIEW · EDIT TARGETS HERE</small>{sections.map((section,index)=>{const sectionItems=data.items.filter(i=>i.area===area&&i.section===section&&!i.optional&&isAction(i));const optional=data.items.filter(i=>i.area===area&&i.section===section&&i.optional&&isAction(i)).length;const reflective=data.items.filter(i=>i.area===area&&i.section===section&&!isAction(i)).length;const done=sectionItems.filter(isDone).length;return <div className="toc-row" key={section}><button className="toc-jump" onClick={()=>document.getElementById(sectionId(section))?.scrollIntoView({behavior:"smooth",block:"start"})}><span>0{index+1}</span><div><b>{section}</b><small>{sectionItems.length?`${done} of ${sectionItems.length} required actions complete`:"No scored actions"}{optional?` · ${optional} optional`:""}{reflective?` · ${reflective} guidance/reflection`:""}</small></div><i>↓</i></button><label className="section-target"><span>Desired completion</span><input type="date" value={data.sectionTargets[section]||""} onChange={e=>update({...data,sectionTargets:{...data.sectionTargets,[section]:e.target.value}})}/></label></div>})}</nav></section>;
}
...
function AreaPage({area,data,update,edit}:{area:Area;data:MoveData;update:(d:MoveData)=>void;edit:(i:MoveItem)=>void}) {
  const intro={
    ...
    Build:["Build the way there.","A Chicago-first path from new income evidence and stronger credit to the right lease or sublet—and a soft landing."],
  ...
  const compass={
    ...
    Build:{question:"WHAT BUILD PROTECTS",focus:"WHAT BELONGS IN BUILD",permission:"THE SEQUENCE TO REMEMBER",questionText:"New income, housing approval, a home that supports daily life, and a softer first month.",focusText:"A remote or Chicago-hybrid search, the 625+ credit path, verified property terms, documents, and move preparation.",permissionText:"Income and credit move in parallel. Commit money and logistics only when the property, address, and evidence are real."},
  ...
  return <div className={`page area-page area-${area.toLowerCase()}`}>
    ...
    {areaSections[area].filter(section=>!(area==="Become"&&section==="My Fuel Source")).map((section,index)=>{
      ...
      const items=data.items.filter(i=>i.area===area&&i.section===section&&!i.parentId&&isAction(i)&&(!data.hideCompleted||!isDone(i)));
      ...
      return <details id={sectionId(section)} className={`section-card section-anchor ${locked?"locked":""}`} key={section} open={index<2&&!locked}>
        ...
        <div className="section-body">{items.map(item=><ItemRow key={item.id} item={item} all={data.items} edit={edit}/>)}{reflections.length>0&&<aside className="section-reflections"><small>DISCOVERY & INNER WORK · NOT SCORED</small><p>These pieces produce clarity, not productivity points.</p>{reflections.map(reflection=><button key={reflection.id} onClick={()=>edit(reflection)}><span>✦</span><div><b>{reflection.title}</b><small>{reflection.description}</small></div><i>→</i></button>)}</aside>}{references.length>0&&<aside className="section-guidance"><small>DECISION GUIDANCE · NOT ACTION ITEMS</small><p>Keep these in view when making the actions above. They do not affect completion.</p>{references.map(reference=><button key={reference.id} onClick={()=>edit(reference)}><span>PSA</span><div><b>{reference.title}</b><small>{reference.description}</small></div><i>→</i></button>)}</aside>}{!locked&&<button className="add-row" onClick={()=>add(section)}>＋ Add something gently</button>}{locked&&<p className="locked-copy">There’s nothing to do here yet. A confirmed address and Move Day will unlock these logistics.</p>}</div>
      </details>;
    })}
  </div>;
}
...
function ItemRow({item,all,edit}:{item:MoveItem;all:MoveItem[];edit:(i:MoveItem)=>void}){const blocked=blocker(item,all);return <button className={`item-row ${isDone(item)?"item-completed":""}`} onClick={()=>edit(item)}><span className={`status-dot ${isDone(item)?"done":""}`}>{isDone(item)?"✓":"○"}</span><span><b>{item.title}</b><small>{blocked || item.description || `${item.status} · ${item.timing}`}</small><span className="item-reason"><strong>Why:</strong> {itemReason(item,all)}</span></span><em>{item.optional?"Optional":blocked?"Waiting":item.status}</em><i>→</i></button>}
  ...
  return <div className="modal-backdrop cosmic-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)onClose()}}>
    <form className={`modal cosmic-editor editor-${draft.area.toLowerCase()}`} onSubmit={e=>{e.preventDefault();onSave({...draft,updatedAt:new Date().toISOString().slice(0,10)})}}>
      ...
      <section className="editor-form">
        <div className="modal-head"><div><span>{draft.kind==="Reference"?"DECISION GUIDANCE · NOT AN ACTION":draft.kind==="Reflection"?"DISCOVERY & INNER WORK · NOT SCORED":item.title?"EDITING A LIVING PIECE":"ADDING A NEW PIECE"}</span><h2>{item.title?draft.title||"Untitled piece":"Add something"}</h2></div><button type="button" onClick={onClose}>×</button></div>
        ...
        {references.length>0&&<aside className="attached-reference"><small>KEEP IN VIEW WHILE DECIDING</small><p>These are reminders, permissions, or decision criteria—not extra work.</p>{references.map(reference=><button type="button" key={reference.id} onClick={()=>onOpen(reference)}><span>PSA</span><div><b>{reference.title}</b><small>{reference.description}</small></div><i>→</i></button>)}</aside>}
        <label><span>What kind of piece is this?</span><select value={draft.kind||"Action"} onChange={e=>setDraft({...draft,kind:e.target.value as MoveItem["kind"]})}><option value="Action">Action — something with a clear finish</option><option value="Reflection">Discovery or inner work — clarity, not a score</option><option value="Reference">Guidance / PSA — keep in view</option></select></label>
        {draft.kind!=="Action"?<div className="reference-notice"><b>{draft.kind==="Reflection"?"This is discovery or inner work.":"This is guidance, not an action item."}</b><span>It remains available for decisions and never changes your completion percentage.</span></div>:<><div className="editor-status-row"><label><span>Status</span><select value={draft.status} onChange={e=>setDraft({...draft,status:e.target.value as Status})}>{statuses.map(s=><option key={s}>{s}</option>)}</select></label><label><span>Timing</span><select value={draft.timing} onChange={e=>setDraft({...draft,timing:e.target.value as Timing})}>{timings.map(t=><option key={t}>{t}</option>)}</select></label></div><label className="optional-toggle"><input type="checkbox" checked={!!draft.optional} onChange={e=>setDraft({...draft,optional:e.target.checked})}/><span><b>Optional action</b><small>Visible and usable, but excluded from required progress and suggestions.</small></span></label></>}
